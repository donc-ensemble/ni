...
export default async function decorate(block) {
  ...
  productCards.forEach((card) => {
    ...
    if (anchorElement) {
      ...
      card.dataset.href = url; // Store the URL

      // Hide the div containing the <a> element using a class for better performance
      const divToHide = anchorElement.closest('div');
      if (divToHide) {
        divToHide.classList.add('hidden-link-container');
      }

      // Add a class to the card to indicate it's clickable and apply cursor via CSS
      card.classList.add('clickable-card');
    }
  });

  // Use event delegation on the main block to handle clicks for all cards
  block.addEventListener('click', (event) => {
    // Find the closest clickable card ancestor of the clicked element
